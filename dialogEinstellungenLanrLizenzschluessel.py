...
class EinstellungenProgrammerweiterungen(QDialog):
    def __init__(self, configPath):
        ...
        if re.match(reLizenzschluessel, self.lineEditLizenzschluessel.text()) != None and gdttoolsL.GdtToolsLizenzschluessel.nochTageGueltig(self.lizenzschluessel) <=0:
            mb = QMessageBox(QMessageBox.Icon.Information, "Hinweis von InrGDT", "Der eingetragene Lizenzschlüssel ist abgelaufen.", QMessageBox.StandardButton.Ok)
            mb.exec()  
    ...
    def accept(self):
        if self.lineEditLanr.text() == "" and self.lineEditLizenzschluessel.text() == "":
            ...
            mb = QMessageBox(QMessageBox.Icon.Information, "Hinweis von InrGDT", "Die LANR ist ungültig.", QMessageBox.StandardButton.Ok)
            mb.exec()
            self.lineEditLanr.setFocus()
            self.lineEditLanr.selectAll()
        elif re.match(reLizenzschluessel, self.lineEditLizenzschluessel.text()) == None or (not gdttoolsL.GdtToolsLizenzschluessel.lizenzErteilt(self.lineEditLizenzschluessel.text().upper(), self.lineEditLanr.text(), gdttoolsL.SoftwareId.INRGDT) and not gdttoolsL.GdtToolsLizenzschluessel.lizenzErteilt(self.lineEditLizenzschluessel.text().upper(), self.lineEditLanr.text(), gdttoolsL.SoftwareId.INRGDTPSEUDO)):
            mb = QMessageBox(QMessageBox.Icon.Information, "Hinweis von InrGDT", "Die LANR/Lizenzschlüssel-Kombination ist ungültig.", QMessageBox.StandardButton.Ok)
            mb.exec()
            self.lineEditLizenzschluessel.setFocus()
            self.lineEditLizenzschluessel.selectAll()
        else:
