...
class EinstellungenImportExport(QDialog):
    def __init__(self, configPath):
        ...
        self.checkboxTextListe = ["Allgemein", "GDT-Einstellungen", "Dosierungen", "BenutzerInnen", "LANR/Lizenzschlüssel"]
    ...
    def accept(self):
        if self.buttonBox.button(QDialogButtonBox.StandardButton.Ok).text() == "Importieren...":
            ...
            fd.setModal(True)
            fd.setNameFilters(["InrGDT-Einstellungsdateien (*.ied)"])
            ...
            if fd.exec() == 1:
                ...
                if "Allgemein" in configImport.sections() or "GDT" in configImport.sections() or "Marcumar" in configImport.sections() or "Benutzer" in configImport.sections() or "Erweiterungen" in configImport.sections():
                    i=0
                    ...
                    try:
                        ...
                        self.done(1)
                        mb = QMessageBox(QMessageBox.Icon.Warning, "Hinweis von InrGDT", "Die Einstellungen wurden erfolgreich importiert. Damit diese wirksam werden, muss InrGDT neu gestartet werden.\nSoll InrGDT neu gestartet werden?", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
                        ...
                        mb = QMessageBox(QMessageBox.Icon.Warning, "Hinweis von InrGDT", "Fehler beim Importieren der Einstellungen: " + str(e), QMessageBox.StandardButton.Ok)
                        mb.exec()
                else:
                    mb = QMessageBox(QMessageBox.Icon.Warning, "Hinweis von InrGDT", "Die Datei " + datei + " ist keine gültige InrGDT-Konfigurationsdatei", QMessageBox.StandardButton.Ok)
                    mb.exec()
        else:
            fd = QFileDialog(self)
            ...
            if fd.exec() == 1:
                ...
                dateiname = datetime.datetime.strftime(datetime.datetime.now(), "%Y%m%d%H%M%S") + "_InrGdtEinstellungen.ied"
                ...
                try:
                    with open(datei, "w") as exportfile:
                        ...
                        mb = QMessageBox(QMessageBox.Icon.Information, "Hinweis von InrGDT", "Die gewünschten Einstellungen wurden erfolgreich unter dem Namen " + dateiname + " exportiert.", QMessageBox.StandardButton.Ok)
                        mb.exec()
                        self.done(1)
                except Exception as e:
                    mb = QMessageBox(QMessageBox.Icon.Warning, "Hinweis von InrGDT", "Fehler beim Exportieren der Einstellungen: " + str(e), QMessageBox.StandardButton.Ok)
                    mb.exec()
